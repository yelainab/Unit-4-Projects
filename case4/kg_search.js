...
function init() {
   // displays the title of the puzzle
   ...
   // changes cursor styles and highlights on mouse down 
   ...
   document.getElementById("wordSearchTable").onmouseup = function() {
      ...
      var solved = true;
      // displays a message if all the words are crossed out
   ...
   // displays the solution in purple when show solution is clicked
...
// highlights the letters not solved organge 
...
// function compares the words on the list to the highlighted words 
function checkLetters() {
   var currentLetters = document.getElementById("pickedLetters").value;
   var wordList = document.querySelectorAll("ul#wordSearchList li");
   // stickes out the words on the list when found on the puzzle
   for (var i = 0; i < wordList.length; i++) {
   ...
   // turns correct words from orange to green 
   for (var i = 0; i < allCells.length; i++) {
